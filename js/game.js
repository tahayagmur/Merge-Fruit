...
// Ödüllü reklam butonları – boost adını HTML'den al
const BOOST_NAMES = { shake: 'Sarsıntı', bomb: 'Bomba', swap: 'Dönüştürücü', upgrade: 'Seviye Atlama' };
document.querySelectorAll('.buy-ad-btn, .store-buy-ad').forEach(btn => {
    btn.addEventListener('click', (e) => {
        const type = e.target.getAttribute('data-type');
        showRewardedAd(type, BOOST_NAMES[type] || type);
    });
});

// Bomb Mechanics Pointer Handler
function handlePointerDown(e) {
    if (!bombMode || !isGameStarted || isGameOver) return;
    ...
    const allBodies = Composite.allBodies(world).filter(b => b.label === 'fruit' && b.customData && b.customData.hasDropped);
    
    for (let b of allBodies) {
        const dx = b.position.x - x;
        const dy = b.position.y - y;
        if (dx*dx + dy*dy <= b.circleRadius * b.circleRadius) {
            // Destroy fruit
            Composite.remove(world, b);
            spawnParticles(b.position.x, b.position.y, b.customData.color);
            boosts.bomb--;
            bombMode = false;
...
document.getElementById('btn-shake').addEventListener('click', () => {
    if (boosts.shake <= 0) {
        if (isGameStarted && !isGameOver) openStore(true);
        return;
    }
...
document.getElementById('btn-swap').addEventListener('click', () => {
    ...
    if (!isGameStarted || isGameOver || isDropping) return;
    boosts.swap--;
    saveGameState();
    updateHUDs();
    setNextFruit();
...
document.getElementById('btn-upgrade').addEventListener('click', () => {
    ...
    if (!isGameStarted || isGameOver || isDropping) return;
    if (nextFruitLevel < FRUITS.length - 2) {
        boosts.upgrade--;
        saveGameState();
        updateHUDs();
        nextFruitLevel++;
...
let bgParticlesInterval;
const bgParticlesContainer = document.getElementById('bg-particles');

function startMenuParticles() {
    if (bgParticlesInterval) return;
    spawnMenuParticle();
    bgParticlesInterval = setInterval(spawnMenuParticle, 800);
}
...
function stopMenuParticles() {
    ...
    bgParticlesContainer.innerHTML = '';
}
...
function spawnMenuParticle() {
    ...
    if (fruit.imgSource) {
        el.innerHTML = `<img src="${fruit.imgSource}" style="width:100%; height:100%; object-fit:contain;">`;
    } else {
        el.innerText = fruit.emoji;
        el.style.fontSize = size + 'px';
        el.style.textAlign = 'center';
        el.style.lineHeight = size + 'px';
    }
    
    bgParticlesContainer.appendChild(el);
    
    setTimeout(() => {
        if (el.parentNode === bgParticlesContainer) {
            bgParticlesContainer.removeChild(el);
        }
    }, duration * 1000);
}
